...
from crew.views import SautaiAgentsCrew
import requests
...
# def run_crew_meal_plan(request):
#     user_id = request.user.id
#     user = get_object_or_404(CustomUser, id=user_id)

#     # Fetch approved meal plans and associated meals
#     approved_meal_plans = MealPlan.objects.filter(user=user, is_approved=True)
#     meal_plan_meals = []
#     for meal_plan in approved_meal_plans:
#         for meal_plan_meal in meal_plan.mealplanmeal_set.all():
#             meal = meal_plan_meal.meal
#             meal_plan_meals.append({
#                 'meal_id': meal.id,
#                 'meal_name': meal.name,
...
#     try:
#         # Initialize the crew with user data
#         crew = SautaiAgentsCrew()
#         logger.info("Initialized SautaiAgentsCrew with user data")

#         # Run the crew and get results
#         results = crew.updated_crew().kickoff(inputs=user_data)
#         logger.info("Crew kickoff completed")

#         # Return the response with the crew plan output
#         return Response({"crew_plan": results}, status=200)
...
# @api_view(['GET', 'POST'])
# @permission_classes([IsAuthenticated, IsCustomer])
# def api_crew_meal_plan(request):
...
def run_crew_meal_plan(request):
    ...
    try:
        # Prepare the context and question for the OpenAI API
        app_context = "user's dietary preferences and approved meal plans"
        question = "Generate a shopping list for the meals"

        # Call the OpenAI API to generate the shopping list
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {
                    "role": "system",
                    "content": f"Given the user's dietary preferences and approved meal plans, generate a shopping list for the following meals: {user_data['meal_plan_meals']}. The user has the following dietary preferences: {user_data['dietary_preference']}, allergies: {user_data['allergies']}, custom allergies: {user_data['custom_allergies']}, and {user_data['custom_dietary_preference']}."
                }
            ]
        )
        ...
        print(f'Shopping list generated by run crew using OpenAI API: {response_content}')
        logger.info("Shopping list generated using OpenAI API")

        # Return the response with the shopping list
        return Response({"shopping_list": response_content}, status=200)

    except Exception as e:
        logger.error(f"Error during shopping list generation: {e}")
        return Response({"error": str(e)}, status=500)
...
def api_crew_meal_plan(request):
    try:
        ...
        # Collect user data for OpenAI API input
        user_data = {
            'allergies': user.allergies,
            'custom_allergies': user.custom_allergies,
            'dietary_preference': user.dietary_preference,
        ...
        # Interpret the model's response
        response_content = response.choices[0].message.content.strip()
        print(f'Shopping list generated using OpenAI API: {response_content}')
        logger.info("Shopping list generated using OpenAI API")
